<script lang="ts">
	...
	function toggleCommittedSelectionLine(hunk: DiffHunk, line: LineId, allDeltaLines: LineId[]) {
		...
		const newLines = alreadySelected
			? current.lines.filter((l) => !sameLine(l, line))
			: [...current.lines, line];

		// If every delta line is selected, represent it as full selection (empty list).
		const allSelected =
			allDeltaLines.length > 0 &&
			allDeltaLines.every((l) => newLines.some((selectedLine) => sameLine(selectedLine, l)));
					...
					<div
						...
						use:draggableChips={{
							label: selectedHunkHeaders
								? `${hunkHeader} (${selectedLineCount}\u00A0line${
					...
					>
						<HunkDiff
							...
							onLineClick={(p) => {
								...
								// Only delta lines are selectable for committed hunks.
								if (
									(line.newLine === undefined && line.oldLine === undefined) ||
									(line.newLine !== undefined && line.oldLine !== undefined)
								) {
									return;
								}
