...
test('should discard an untracked added file via context menu', async ({ page, context }, testInfo) => {
	const workdir = testInfo.outputPath('workdir');
	const configdir = testInfo.outputPath('config');
	gitbutler = await startGitButler(workdir, configdir, context);
	...
	// The file should appear on the uncommitted changes area
	const uncommittedChangesList = getByTestId(page, 'uncommitted-changes-file-list');
	const fileItem = uncommittedChangesList.getByTestId('file-list-item').filter({ hasText: fileName });
	await expect(fileItem).toBeVisible();
	await fileItem.click();
